...
resource "azurerm_windows_web_app" "hro-api" {
  ...
  app_settings = {
    ...
    APPOINTMENT_CHANGED_SMS_NOTIFY_TEMPLATE_ID      = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.appointment_changed_sms_notify_template_id.id})"
    APPOINTMENT_CHANGED_EMAIL_NOTIFY_TEMPLATE_ID    = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.appointment_changed_email_notify_template_id.id})"
